...
export class SundialView implements SubView {
    ...
    constructor() {
        ...
        this.returnArrow= new ReturnArrow( STAGE_WIDTH*0.01, STAGE_HEIGHT*0.88,
            () => {this.popFromScreen();}, "returnArrow");

        let centerX = STAGE_WIDTH/2;
        let centerY = STAGE_HEIGHT/3;
        this.dial = new PuzzleDial(centerX, centerY, "puzzleDial", 0);
        this.input = new NumericInput(400,600, "input", 4.20);

        this.infoTray = new InfoTray(STAGE_WIDTH/2, 0, "measureTray");
